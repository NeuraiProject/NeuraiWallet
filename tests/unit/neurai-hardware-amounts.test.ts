...
import { encodeDestinationScript } from '@neuraiproject/neurai-sign-esp32/react-native';
...
test.each([false, true])('builds a legacy PSBT with exact odd large prevout and outputs (sendMax=%s)', async sendMax => {
  ...
  const result = await wallet.buildUnsignedSend(recipient, 100000000n, { feeRate: 1, sendMax });
...
it('rejects an unsafe PQ input instead of rounding signing metadata', async () => {
  ...
  const { wallet } = fixture(10000000000000001n, 'pq', address);
  await expect(wallet.buildUnsignedSend(recipient, 1n, { feeRate: 1 })).rejects.toThrow('does not support this input amount exactly');
});
...
it('builds safe PQ signing metadata and conserves every satoshi', async () => {
  ...
  const result = await wallet.buildUnsignedSend(recipient, 100000001n, { feeRate: 1 });
  expect(result.inputs![0].amount).toBe(Number.MAX_SAFE_INTEGER);
...
it('accounts for a one-satoshi dust change in the exact fee', async () => {
  ...
  const result = await wallet.buildUnsignedSend(recipient, 100000000n, { feeRate: 1 });
  const psbt = Psbt.fromBase64(result.psbtBase64!);
